...
export default function DashboardPage() {
  ...
  const fetchDashboardData = async () => {
    try {
      const [incomesData, expensesData, debtsData] = await Promise.all([
        getIncomes(),
        getExpenses(),
        getDebts(),
      ])
      ...
      const totalDebtsBalance = debtsData.reduce((acc, curr) => acc + Number(curr.current_balance), 0)
      const totalDebtMinPayments = debtsData.reduce((acc, curr) => acc + Number(curr.minimum_payment), 0)

      const summary = { totalIncome, totalExpenses, totalDebtsBalance, totalDebtMinPayments }
      setMetrics(summary)
  ...
  if (loading) {
    return (
      <div className="p-8 bg-gray-950 min-h-screen text-gray-400 flex items-center justify-center">
  ...
  return (
    <div className="p-6 md:p-8 bg-gray-950 min-h-screen text-gray-100 space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-gray-900 border border-gray-800 p-5 rounded-2xl shadow-sm">
          <p className="text-xs text-gray-400 font-medium">Gastos Totales</p>
        ...
        <div className="bg-gray-900 border border-gray-800 p-5 rounded-2xl shadow-sm">
          <p className="text-xs text-gray-400 font-medium">Flujo Neto Disponible</p>
          <p
        ...
        <div className="bg-gray-900 border border-gray-800 p-5 rounded-2xl shadow-sm">
          <p className="text-xs text-gray-400 font-medium">Saldo Total en Deudas</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-gray-900 border border-gray-800 p-6 rounded-2xl flex flex-col justify-between shadow-lg">
          <div>
            <h3 className="font-semibold text-sm text-gray-300">Nivel de Salud Financiera</h3>
            <p className="text-xs text-gray-500 mt-1">Calculado en base a liquidez y reglas de endeudamiento</p>
          </div>
        ...
        <div className="lg:col-span-2 bg-gray-900 border border-gray-800 p-6 rounded-2xl shadow-lg space-y-6">
          <div>
            <h3 className="font-semibold text-sm text-gray-200">Diagnóstico del Motor Financiero</h3>
          ...
          <div className="space-y-3">
            <h4 className="text-xs uppercase tracking-wider text-gray-500 font-semibold">Alertas activas</h4>
            {report?.alerts.length === 0 ? (
              <p className="text-xs text-gray-400 italic">Sin alertas ni riesgos detectados actualmente.</p>
            ) : (
          ...
          <div className="space-y-3 border-t border-gray-800 pt-4">
            ...
            {report?.recommendations.length === 0 ? (
              <p className="text-xs text-gray-400 italic">Sigue manteniendo la gestión actual de tu capital.</p>
            ) : (
